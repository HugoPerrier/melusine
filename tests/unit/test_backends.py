...
from melusine.backend import reset_backend, switch_backend
...
def test_check_debug_flag_dict():
    ...
    assert backend.check_debug_flag(dict_data) is False

    dict_data = {"debug": False}
    assert backend.check_debug_flag(dict_data) is False

    dict_data = {"debug": True}
    assert backend.check_debug_flag(dict_data) is True
...
def test_check_debug_flag_pandas():
    ...
    assert backend.check_debug_flag(df) is False

    df.debug = False
    assert backend.check_debug_flag(df) is False

    df.debug = True
    assert backend.check_debug_flag(df) is True
...
def test_add_fields_dict():
    ...
    data = backend.add_fields(dict_data1, dict_data2)
...
def test_pandas_backend(backend_base_data, backend_testcase, progress_bar):
    ...
    backend = PandasBackend(progress_bar=progress_bar, workers=1)

    df_base = pd.DataFrame(backend_base_data)

    df_expected = pd.DataFrame(expected_data)
    df_transform = backend.apply_transform(
...
def test_pandas_backend_multiprocess(backend_base_data, backend_testcase):
    ...
    backend = PandasBackend(progress_bar=False, workers=2)
